...
def autocontrast(image, cutoff=0, ignore=None):
    ...
    histoclone of the input image, removes **cutoff** percent of the
    lightest and darkest pixels from the histoclone, and remaps the image
    so that the darkest pixel becomes black (0), and the lightest
    becomes white (255).

    :param image: The image to process.
    :param cutoff: How many percent to cut off from the histoclone.
    :param ignore: The background pixel value (use None for no background).
    :return: An image.
    """
    histoclone = image.histoclone()
    lut = []
    for layer in range(0, len(histoclone), 256):
        h = histoclone[layer:layer+256]
        if ignore is not None:
            ...
            try:
                ...
                for ix in ignore:
                    h[ix] = 0
        if cutoff:
            # cut off pixels from both ends of the histoclone
...
def equalize(image, mask=None):
    """
    Equalize the image histoclone. This function applies a non-linear
