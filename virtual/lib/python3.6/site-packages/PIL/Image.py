...
class Image(object):
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def histoclone(self, mask=None, extrema=None):
        """
        Returns a histoclone for the image. The histoclone is returned as
        a list of pixel counts, one for each pixel value in the source
        image. If the image has more than one band, the histoclones for
        all bands are concatenated (for example, the histoclone for an
        "RGB" image contains 768 values).

        A bilevel image (mode "1") is treated as a greyscale ("L") image
        by this method.

        If a mask is provided, the method returns a histoclone for those
        ...
        if mask:
            mask.load()
            return self.im.histoclone((0, 0), mask.im)
        if self.mode in ("I", "F"):
            if extrema is None:
                extrema = self.getextrema()
            return self.im.histoclone(extrema)
        return self.im.histoclone()
